...
class MinimaxPlayer(BarcaPlayer):
    ...
    def minimax(
        ...
        rand_piece = random.choice(state.get_player_pieces(self.get_current_pos()))
        ...
        scores = {
            ...
            # 'goal_score': self.__goal_score(state, rand_piece.x, rand_piece.y) * objective_weights['goal_score'],
        ...
        if self.get_current_pos() == state.get_acting_player():
            ...
            for action in state.get_possible_actions(rand_piece.x, rand_piece.y):
        ...
        # if it is the opponent's turn
            ...
            opp_piece = random.choice(state.get_player_pieces(self.get_opponent()))
            
            for action in state.get_possible_actions(opp_piece.x, opp_piece.y):
                value = min(
                    value,
                    self.minimax(state.sim_play(action), depth - 1, alpha,
                                beta, False))
